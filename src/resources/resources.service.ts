...
import { Resource, ResourceDocument, ResourceGto } from './schemas/resource.schema';
...
export class ResourcesService {
  ...
  async findAll(): Promise<Resource[]> {
    ...
    if (!resources || !resources[0]) throw new HttpException('Resource not found.', 404);
  ...
  async update(newResource: Resource) {
    const resource = await this.resourceModel.findOneAndUpdate({ id: newResource.id }, { ...newResource }).exec();
    if (!resource) throw new HttpException('Resource not found.', 404);
  ...
  async delete(id: number) {
    ...
    if (resource.deletedCount === 0) throw new HttpException('Resource not found.', 404);
